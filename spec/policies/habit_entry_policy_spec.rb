...
RSpec.describe HabitEntryPolicy do
  let(:user) { create(:user) }
  let(:other_user) { create(:user) }
  let(:daily_entry) { create(:daily_entry, user: user) }
  let(:other_daily_entry) { create(:daily_entry, user: other_user) }
  let(:habit_entry) { create(:habit_entry, daily_entry: daily_entry) }
  let(:other_habit_entry) { create(:habit_entry, daily_entry: other_daily_entry) }

  subject { described_class }

  permissions :create? do
    it 'allows user to create entry for own daily entry' do
      expect(subject).to permit(user, habit_entry)
    end

    it 'denies user from creating entry for other user\'s daily entry' do
      expect(subject).not_to permit(user, other_habit_entry)
    end
  end
  ...
  describe 'Scope' do
    before do
      ...
      create_list(:habit_entry, 2, daily_entry: other_daily_entry)
    end

    it 'returns only current user\'s habit entries' do
      scope = described_class::Scope.new(user, HabitEntry.all).resolve
